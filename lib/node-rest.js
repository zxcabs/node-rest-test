...
Test.prototype.end = function (beforeExit, asserts) {
	if (arguments.length === 2) {
		this.callback = function (err) {
			beforeExit(function () {
				if (err) {
					console.log('Schema: ' + utils.inspect(err.schema, true, 2));
					console.log('Responce Body: ' + utils.inspect(err.res.body, true, 2));
					console.log('Request: ' + utils.inspect(err.req, true, 2));
					asserts.ok(false);
				} else {
					asserts.ok(true);
				}
...
Test.prototype._execute = function () {
	...
	if (!this.callback) {
		this.callback = function(err) {
			if (err) {
				console.log(utils.inspect(err.isValid, true, 2));
				assert.ok(false);
			} else {
				assert.ok(true);
			}
