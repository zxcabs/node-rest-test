...
Test.prototype.end = function (beforeExit, asserts) {
	if (arguments.length === 2) {
		this.callback = function (err) {
			beforeExit(function () {
				if (err) {
					console.log('Error: ' + utils.inspect(err.error, false, 5));
...
Test.prototype._execute = function () {
		...
		assert.response(self.app, req, 
			...
			function (res) {
				...
				if (!error) {
					...
					next({res: res, req: req, schema: schema, error: error});
				}
	...
	if (!this.callback) {
		this.callback = function(err) {
			if (err) {
				console.log(utils.inspect(err.error, false, 5));
				assert.ok(false);
			} else {
				assert.ok(true);
			}
